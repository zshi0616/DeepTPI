...
class BaseDetector(object):
    ...
    def run(self, graph):
        ...
        output, forward_time, node_embedding = self.process(graph, return_time=True)
        ...
        return {'results': output, 'tot': tot_time, 'net': net_time,
                'post': post_time, 'embedding': node_embedding}

    def process(self, graph, return_time=False):

        with torch.no_grad():
            output, node_embedding = self.model(graph)
            output = output[0]
        ...
        if return_time:
            return output, forward_time, node_embedding
        else:
