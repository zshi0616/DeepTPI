...
----------------------------------------------------------------------*/
VEC random_fsim(int iNoGate, int iNoPI, int iNoPO, int iMaxLevelAdd2, int iStem, GATEPTR *pStem, level *LFSR,
				int limit, int maxbit, int maxdetect, int *ntest, int *npacket, int *nbit, FILE *test, int *ndetect)
{
	int iteration = 0, max_iteration = 0;
	int iter_detected = 0;
	int diff_level = 0, max_diff_level = 0;
	int i, j;
	int profile[BITSIZE];
	VEC namelist;
	*ndetect = 0;
	...
	if (limit < 0){
		printf("[INFO] Incremental Random Test Mode\n");
	}
	else{
		printf("[INFO] Random Test with %d patterns \n", limit);
	}
	...
	while (iteration < 256)
	{
		...
		for (i = 0; i < iNoPI; i++) g_net[i]->output1 = g_net[i]->output = LFSR[i];
		evalGatesFromFreeStack();
		for (i = 0; i < maxbit; i++) profile[i] = 0;
		
		int new_detected_cnt = 0;
		int profile_cnt = 0;
		int iter_detected = Fault1_Simulation_NameList(iNoGate, iMaxLevelAdd2, iNoPI, iNoPO, iStem, pStem, maxbit, profile, &namelist);
		...
		if (iter_detected > 0)
		{
			...
			// printf("%d/%d: %d\n", iteration, limit, ndetect);
			if (*ndetect >= maxdetect)
			{
				break;
			}
			if (limit >= 0 && (*ntest) >= limit){
				break;
			}
		}
		else
		...
		for (i = 0; i <= g_iSStack; i++) g_dynamicStack[i]->cobserve = ALL0;
		if (g_iUpdateFlag)
		{
...
VEC random_sim(int nog, int nopi, int nopo, int LEVEL, int nstem, GATEPTR *stem, level *LFSR, int limit, 
			   int maxbit, int maxdetect, int *ntest, int *npacket, int *nbit, FILE *test, int *ndetect)
{
	return(random_fsim(nog, nopi, nopo, LEVEL, nstem, stem, LFSR, limit, maxbit, maxdetect, ntest, npacket, nbit, test, ndetect));
...
int tgen_sim(int iNoGate, int iMaxLevelAdd2, int iNoPI, int iNoPO, int iStem, GATEPTR *pStem, int iNoPatterns,
	int iArrProfile[], VEC *detected_list)
{
	//Return iNoDetected
	...
	if (_MODE_SIM == 'f') //Default FSIM !!!
	{
		...
		return(Fault0_Simulation_Namelist(iNoGate, iMaxLevelAdd2, iNoPI, iNoPO, iStem, pStem, 1, iArrProfile, detected_list));
	}
...
VEC testgen(int iNoGate, int iNoPI, int iNoPO, int iMaxLevelAdd2, int iMaxBitSize, int iStem, GATEPTR *pStem,
			int iMaxBackTrack, int bPhase2, int *piNoRedundant, int *piNoOverBackTrack, int *piNoBackTrack,
			int *piNoPatterns, int *piPacket, int *piBit, double *plfFanTime, FILE *fpTestFile, int *ndetect)
...
// 			}
		...
		else if (iState == TEST_FOUND) //////////////////////////Default !!!
		{
			...
			*/
			...
			//piNoPatterns NO USE !!!
			iArrProfile[0] = tgen_sim(iNoGate, iMaxLevelAdd2, iNoPI, iNoPO, iStem, pStem, *piNoPatterns, iArrProfile, &namelist);
			// iNoDetected += iArrProfile[0];
			*ndetect += iArrProfile[0];
