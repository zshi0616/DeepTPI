...
int Fault1_Simulation_NameList(int iNoGate, int iMaxDPI, int iNoPI, int iNoPO, int iStem, GATEPTR pStem[],
	int iBit, int iArrNoDetected[], VEC *detected_list)
{
	//Return iNoDetected
	//iNoDetected = iArrNoDetected[0]
	//Always !!  iBit == 1
	...
	for (i = 0; i <= g_evalGatesStack.last; i++) //don't pop g_evalGatesStack
	{
		...
		if (pGate->type == PO)
		{
			iNoDetected += updateWholeStemObserveAndDetect_NameList(pGate, &g_iUpdateFlag, TRUE, iBit, iArrNoDetected, detected_list); //pGate->nfault == 0 ----> g_iUpdateFlag = 1
			//g_iUpdateFlag ONLY used in Fault0_Simulation !!!
		}


		//output >= 2 (FANOUT)		bIsPO == FALSE
		else if ((pGate->observe = updateWholeStemObserveAndDetect_NameList(pGate, &g_iUpdateFlag, FALSE, iBit, iArrNoDetected, detected_list)) != ALL0) //g_iUpdateFlag no change!!!
		{
			pDomiGate = (pGate->u_path == NULL) ? NULL : pGate->u_path->ngate;
			if ((pGate->observe = faultSimForStemGate(pGate, pGate->observe, pDomiGate, iMaxDPI)) != ALL0)
			{
			...
			while ((pFault = (*ppFault)) != NULL) //For every pFault in pGate->dfault
			{
				if ((pFault->observe & pGate->observe) != ALL0) //Core sentence
				{
					...
					for (i = iBit - 1; i >= 0; i--) // 1 <= iBit <= 32
					{
						...
						{
							// printf("Detected: %s \n", pFault->gate->hash->symbol);
							FAULT_RECORD tmp_record(pFault->gate->index, pFault->type);
							detected_list->push_back(tmp_record);
							++iArrNoDetected[i];
							break;
						}
					}
...
int updateWholeStemObserveAndDetect_NameList(GATEPTR pStemGate, status *piUpdateFlag, status bIsPO, int iBit, int iArrNoDetected[], VEC* detected_list) //ftp_reverse
{
	//INPUT:  pStemGate
	//OUTPUT:  iNoDetected |(iObserve(pStemGate->observe) & ppFault)
	//iBit == 1
	...
	while (!is_empty(g_stack))
	{
		...
		if (pGate->nfault > 0)
		{
			...
			{
				//Always:  iBit == 1
				iNoDetected += updateFaultObserveByPOGate_NameList(pGate, piUpdateFlag, iBit, iArrNoDetected, detected_list);
				//pGate->nfault == 0 ----> *piUpdateFlag == 1
			}
			else //FFR Gates (FANOUT or other Gate in stem)
			{
...
int updateFaultObserveByPOGate_NameList(register GATEPTR pGate, status *piUpdateFlag, int iBit, int iArrProfile[], VEC* detected_list) //pcheck_po
{
	//INPUT:  pGate
	//OUTPUT:  iNoDetected & iArrProfile & pFault->detected
...
int Fault0_Simulation_Namelist(int iNoGate, int iMaxDPI, int iNoPI, int iNoPO, int iStem, GATEPTR pStem[],
	int iBit, int iArrNoDetected[], VEC *detected_list)
{
	//Return iNoDetected
	//iNoDetected = iArrNoDetected[0]
