...
char g_strDetectedFaultFileName[100] = "";
...
int num_pattern_counts = -1;

FILE *fpFaultFile;
FILE *fpUndetFaultFile;
double lfMemSize;

VEC BIST_fault_record, ATPG_fault_record;
...
// 			}
		...
		if (faultmode == 'd')
		{
			...
			// g_iNoFault = initFaultList_SA0_SA1(g_iNoGate, iNoStemGates, pStemGates);
			//Faults generated !!
		}
		else //faultmode == 'f'
		{
	...
	int bist_detected_cnt = 0;
	int atpg_detected_cnt = 0;
	...
	// 	int ntest = 0;
	// 	int maxdetect = 10000000;
	// 	int randomlimit = num_pattern_counts;
	// 	// ndetect=random_sim(nog, nopi, nopo, LEVEL, nstem, stem, LFSR, randomlimit, maxbits, maxdetect, &ntest, &npacket, &nbit, test);
	// 	BIST_fault_record=random_sim(g_iNoGate, g_iNoPI, g_iNoPO, iMaxLevelAdd2, iNoStemGates, pStemGates, \
	// 						   LFSR, randomlimit, iMaxBitSize, maxdetect, &ntest, &iPacket, &iBit, g_fpTestFile, &bist_detected_cnt);
	// }
	...
	ATPG_fault_record = testgen(g_iNoGate, g_iNoPI, g_iNoPO, iMaxLevelAdd2, iMaxBitSize, iNoStemGates, pStemGates, g_iMaxBackTrack1,
		bPhase2, &iNoRedundant, &iNoOverBackTrack, &iNoBackTrack, &iNoTempPatterns, &iPacket, &iBit, &lfFanTime, g_fpTestFile, &atpg_detected_cnt);
	iNoPatterns = iNoTempPatterns;

	printf("[TPG-INFO] Total Faults: %d\n", g_iNoFault);
	printf("[TPG-INFO] ATPG detected fault count: %d\n", ATPG_fault_record.size());
	printf("[TPG-INFO] ATPG pattern count: %d\n", iNoPatterns);
	printf("[TPG-INFO] Test Coverage: %d / %d = %.2f\% \n", ATPG_fault_record.size(), g_iNoFault, 
			(ATPG_fault_record.size())*1.00 / g_iNoFault * 100);
	printf("=======================\n");
	
	if (g_strDetectedFaultFileName != "")
	{
		freopen(g_strDetectedFaultFileName, "w", stdout);
		for (i=0; i<ATPG_fault_record.size(); i++){
			printf("%s /%d\n", g_net[ATPG_fault_record[i].first]->hash->symbol, ATPG_fault_record[i].second);
		}
		fclose(stdout);
	}
	return 0;
...
int read_option(char option, char *array[], int i, int n)
{
	...
	case 'p':
		sscanf(array[++i], "%d", &num_pattern_counts); break;
	case 'B':
	...
	case 'F':
		strcpy(g_strDetectedFaultFileName, array[++i]); break;
	case 'H':
		_MODE_SIM = 'h'; break;
	case '0':
		fillmode = '0'; break;
