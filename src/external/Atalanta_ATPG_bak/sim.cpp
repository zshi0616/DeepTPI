				...
				int limit, int maxbit, int maxdetect, int *ntest, int *npacket, int *nbit, FILE *test)
{
	...
	while (iteration < max_iteration)
	{
		...
		int profile_cnt = 0;
		int iter_detected = Fault1_Simulation(iNoGate, iMaxLevelAdd2, iNoPI, iNoPO, iStem, pStem, maxbit, profile);
...
int random_sim(int nog, int nopi, int nopo, int LEVEL, int nstem, GATEPTR *stem, level *LFSR, int limit, 
			   int maxbit, int maxdetect, int *ntest, int *npacket, int *nbit, FILE *test)
{
	if (_MODE_SIM == 'f')
	{
...
int testgen(int iNoGate, int iNoPI, int iNoPO, int iMaxLevelAdd2, int iMaxBitSize, int iStem, GATEPTR *pStem,
			...
			int *piNoPatterns, int *piPacket, int *piBit, double *plfFanTime, FILE *fpTestFile, int target_fault_coverage)
...
// FILE *test;
{
	...
	if (target_fault_coverage != -1) {
	    exit_last_fault_cnt = g_iNoFault * (100-target_fault_coverage)/100;
	}
...
//		fprintf(stderr, "iLastUndetectedFault=%d\n", iLastUndetectedFault);
		...
		if (iLastUndetectedFault < exit_last_fault_cnt) {
		    printf("Exit with last faults: %d / %d \n", iLastUndetectedFault, g_iNoFault);
		    break;
		}
	}
