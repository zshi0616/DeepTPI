...
from utils.utils import AverageMeter, collate_fn
...
class ModelWithLoss(torch.nn.Module):
    ...
    def forward(self, batch):
        outputs = self.model.solve_and_evaluate(batch)
        loss = self.loss(outputs).mean()
        loss_stats = {'loss': loss}
        return outputs, loss, loss_stats


class CircuitSATTrainer(object):
    def __init__(
            self, args, model, optimizer=None):
        self.args = args
        self.optimizer = optimizer
        self.loss_stats, self.loss = self._get_losses(args.sat_loss, args.k_step)
    ...
    def run_epoch(self, phase, epoch, dataset):
        ...
        for iter_id, batch in enumerate(dataset):
            ...
            avg_prec_stats['ACC'].update(preds.sum().item()/batch.num_graphs, batch.num_graphs)
        ...
        model_with_loss.model.update_temperature(epoch)
        bar.finish()
    ...
    def _get_losses(self, loss, k_step):
        if loss in _loss_factory.keys():
            loss = _loss_factory[loss](k_step)
        else:
            raise KeyError
        loss_states = ['loss']
