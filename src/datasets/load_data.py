...
def circuit_parse_pyg(x, edge_index, y, use_edge_attr=False, reconv_skip_connection=False, logic_diff_embedding="positional", predict_diff=False, diff_multiplier=10, no_node_cop=False, node_reconv=False, un_directed=False, num_gate_types=9, dim_edge_feature=32):
    '''
    ...
    Parameters:
        ...
        node_reconv : bool, whether use the reconvergence info as the node features.
    ...
    if len(x[0]) <= 3:
        ...
        # add C0 info 
        graph.c0 = torch.tensor(x[:, 4:5], dtype=torch.float)
        # add CO info
        graph.co = torch.tensor(x[:, 5:6], dtype=torch.float)
        # add gt info
        graph.gt = torch.tensor(y, dtype=torch.float)
        # add indices for gate types
        graph.gate = torch.tensor(x[:, 1:2], dtype=torch.float)
        # add mask
        graph.mask = torch.tensor(x[:, 5:6], dtype=torch.float)
        # graph.mask = torch.ones(len(x))
        # graph.mask = torch.tensor(graph.mask, dtype=torch.float)
...
def aig_parse_pyg(iclauses, n_vars, n_clauses):
    '''
