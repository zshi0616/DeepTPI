...
from .load_data import circuitsat_parse_pyg, aig_parse_pyg
    ...
    def process(self):
        ...
        for n_var in range(self.min_n, self.max_n+1):
            ...
            for problems_idx in range(lower_bound, upper_bound):
                ...
                if self.use_aig:
                    graph = aig_parse_pyg(iclauses, n_vars, n_clauses)
                else:
                    graph = circuitsat_parse_pyg(iclauses, n_vars, n_clauses, self.exp_depth)
